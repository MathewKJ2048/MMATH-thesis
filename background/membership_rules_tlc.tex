\section{Rules to write a valid TLA+ model}
...
$MC(V)$ is a membership constraint of V. It specifies a finite set of values that V can take, without references to V'. $MC(V)$ can take three basic forms:

\begin{itemize}
	\item $V = E$
	\item $V \in E$
	\item $MCA(V) \lor MCB(V)$
\end{itemize}
...
\begin{itemize}
	...
	\item $V' \in E(V)$
	\item $UNCHANGED V$
	\item $MCA\_prime(V',V) \lor MCB\_prime(V',V)$
\end{itemize}
...
For TLA+ models with multiple variables $V_1$ to $V_n$, the following rules hold:

\begin{enumerate}
	\item $LC_i(V_i)$ must appear only after $MC_i(V_i)$, for every variable $V_i$. 
	\item $MC_i(V_i)$ can make a reference to $V_j$ in the body, iff $MC_j(V_j)$ appears before $MC_i(V_i)$, for any pair of variables $V_i$ and $V_j$.
	\item Analogous rules hold for the primed forms of the variables.
\end{enumerate}
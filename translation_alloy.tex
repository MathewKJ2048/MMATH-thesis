...
The goal of a translation to TLA+ is that when the translated model is run using TLC, the output produced by TLC can be used to infer the output the Analyzer would have produced, had the original model been run on the Analyzer. A translation is correct, if and only if the Analyzer's outputs correspond to the outputs produced by TLC. A translation is efficient, if the time taken to run a translated model using TLC is lower than the time taken to run the original model using the Analyzer.
...
\WHILE{$state\_queue \neq []$}
    ...
    \STATE $CS\_nexts \gets Next\_MC(s)$
    ...
    \IF{\NOT $flag$ \AND $EXIT\_ON\_DEADLOCK$}

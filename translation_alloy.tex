...
The goal of a translation to TLA+ is that when the translated model is run using TLC, the output produced by TLC can be used to infer the output the Analyzer would have produced, had the original model been run on the Analyzer. A translation is correct, if and only if the Analyzer's outputs correspond to the outputs produced by TLC.
